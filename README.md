...
2.1 **多本书配对随机读**：如果希望多本书按“书-章节”配对随机读，请在 `config.py` 里的 `book_chapters = []` 位置直接改成下面这种结构：
...
配置说明：
- `book`：书籍 ID
- `chapters`：这本书可用的章节 ID 列表
- 一个 `{}` 就代表一本书和它自己的章节集合
- 可以只配 1 本书，也可以配多本书

运行规则：
- 脚本会先随机选一本书，再从这本书对应的 `chapters` 里随机选一章
- 如果 `book_chapters = []` 保持空列表，则不会使用配对模式，而是继续使用旧的 `book` 和 `chapter` 配置
- `chapters` 里的章节 ID 必须属于对应的那本书，不要混填
